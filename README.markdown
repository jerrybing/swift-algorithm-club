...
- 按照自己的风格进行翻译给自己看。
- **重点实践费曼技巧**，个别地方添加自己的理解，可能不全面，不严谨，
- 个别时候的翻译猥琐，只是为了单调的码字节奏时能带来一点儿愉悦，所以17禁！虽然趣味低级，但是不失为放松大脑的好方法啊。
- 翻译顺序，以自己的阅读顺序为准
...
如果你是一个学习计算机科学的学生，需要掌握这些东西来应付考试，或者说你是一个自学主义的码农，想要窥探一下你的作品背后的理论奥妙，那么，少年，你来对地方了。

此项目的目的在于**阐述算法是如何工作的**，专注于让代码清晰简洁化，更具有可读性，而不是实现一个可用于你自己项目的library。总而言之，大部分的代码经过稍加改造后，在自己的项目中使用。

代码支持**Xcode 7.3** and **Swift 2.2**,我们将尽量与最新版本的Swift同步
这是一条持久的项目，
这一场持久战，更多装满算法的弹药将会被添加，敬请期待。:-) 


##重要链接

[什么是算法(algorithms)和数据结构(data structures)?](What are Algorithms.markdown) Pancakes!
